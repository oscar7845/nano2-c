...
//must be complete here because i instantiate it below
struct Nano2RNG{
  uint32_t s;
  int have_spare;
  float spare;
...
//helper 
//carve flat buffer
static inline float* carve(float** base, size_t count){ 
  float* p=*base; *base += count; return p; 
}
...
void model_log_summary(const struct Model* M, const struct Config* c){
  (void)c; //not needed, but keep to match prototype
  ...
  //layout log (matches carving order)
  printf("  tensors:\n");
  printf("    E           : [%d, %d]\n", M->V, M->D);
