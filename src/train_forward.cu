...
extern "C" float nano2_forward_loss(struct Model* M, 
  ...
  //2) TokenMix: pre-LN → attention → residual
  nano2_layernorm_forward(M->buf.x, M->buf.x_ln1,
  			  M->p.ln1_g, M->p.ln1_b,
  			  /*N=*/BT, /*D=*/D, /*eps=*/1e-5f);
  			  ///*mean_out=*/M->buf.ln1_mean, /*invstd_out=*/M->buf.ln1_invstd);
  CUDA_CHECK("ln1");
  ...
  {    
    ...
    // copy x to x_res1, then add attn_out in-place
  ...
  //3) FFN: pre-LN → W1+b1 → GELU → W2+b2 → residual
  ...
  nano2_gemm_f32(0,0, BT, F, D, M->buf.x_ln2, D, M->p.W1, F, M->buf.ff1_pre, F, 1.0f, 0.0f);
  
  // + b1
  {
    int threads = (F>=256)?256:(F>=128?128:64);
    dim3 block(threads,1,1), grid((F+threads-1)/threads,1,1);
    add_bias_inplace_kernel<<<grid, block>>>(M->buf.ff1_pre, M->p.b1, BT, F);
    CUDA_CHECK("b1 add");
  }
  ...
  nano2_gelu_forward(M->buf.ff1_pre, M->buf.ff1, BT*F, /*approximate=*/1);  
  CUDA_CHECK("gelu");
  ...
  //4) LM head (tied): logits = x_res2 @ E^T -> [BT,V]
  ...
  /*dlogits=*/nullptr);
    ...
    //Copy logits[0] row (length V) and y[0] to host and compute CE on CPU.
