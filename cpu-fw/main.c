// cpu-fw/main.c
// Pure CPU forward-only benchmark:
// - Loads config + dataset
// - Builds a tiny 1-layer Transformer-like model on CPU
// - Runs forward pass N times and reports loss + timings.
//
// No CUDA is used here; this is intentionally CPU-only.
...
// ---------------------- Config & Dataset ----------------------
// Reuse same struct layout as config.c / data.c.
...
int nano2_cpu_fw_run(int argc, char** argv){
    MPI_Init(&argc, &argv);
    ...
    if (!x || !y){
        fprintf(stderr, "[cpu-fw] host batch malloc failed\n");
    ...
    if (rank == 0){
        ...
        printf("global tokens/sec (all ranks, CPU fw): %.0f\n", toks_per_s);
    }
...
// Tiny wrapper that guarantees a visible main symbol for the linker.
int main(int argc, char** argv){
    return nano2_cpu_fw_run(argc, argv);
}
