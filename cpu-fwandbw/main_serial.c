...
static void a_s(void){
    const int outer = 2000;
    const int inner = 2000;
    volatile double sink = 0.0;

    for (int i = 0; i < outer; ++i){
        for (int j = 0; j < inner; ++j){
            double x = (double)i * 0.001 + (double)j * 0.0001;
            sink += sin(x) * cos(x);
        }
    }
    (void)sink; // prevent "unused variable" + keep work from being optimized out
}
    ...
    for (int b = 0; b < B; ++b){
        ...
        for (int t_q = 0; t_q < T; ++t_q){
            ...
            for (int t_k = 0; t_k < T; ++t_k){
                ...
                float dP_val = 0.0f;
                for (int d0 = 0; d0 < D; ++d0){
                    float dctx_val = d_ctx[ctxbase + d0];
                    dP_val += dctx_val * v[vbase + d0];
                    dv[vbase + d0] += dctx_val * probs[idx];
                }
                d_probs[idx] = dP_val;
            }
        }
...
int main(int argc, char** argv){
    ...
    printf("nano2-cpu-fw+bw-serial: world=%d (no MPI)\n", world);
    ...
    printf("config: %s\n", config_path);
    config_log(&cfg);
    printf("[cpu-fw+bw-serial] forward+backward iters: %d\n", iters);
    ...
    if (!x || !y){
        fprintf(stderr, "[cpu-fw+bw-serial] host batch malloc failed\n");
        free(x);
        free(y);
        cpu_model_free(&M);
        dataset_free(&train_ds);
    ...
    printf("tokens/sec (serial CPU fw+bw): %.0f\n", toks_per_s);
