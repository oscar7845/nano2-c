// Forward-only benchmark executable: MPI + CUDA device selection, load config & data,
// run nano2_forward_loss multiple times and print loss + timings.
...
// Model + forward
struct Model; // opaque here, defined in model.c
...
// parse number of forward iterations: --fw-iters=N  (default 1)
...
int main(int argc, char** argv){
    ...
    if (rank == 0){
        printf("nano2-fw: world=%d local_rank=%d/%d device=%d\n",
               world, local_rank, local_size, dev);
    }

    // parse config + fw-iters
    ...
    // preview first few tokens on rank 0
    if (rank == 0){
    ...
    // make sure all ranks are ready before timing
    ...
    // reduce timing across ranks: take max wall time
    double local_ms = (double)ms;
    double max_ms   = 0.0;
